import numpy as np
import torch
from ..utils import as_complex, get_activ_fn
...
class MREPINN(torch.nn.Module):
    def __init__(self, example, omega, activ_fn='ss', **kwargs):
        ...
        x_center = example['x_center']
        x_extent = example['x_extent']

        self.u_loc = torch.tensor(example['u_loc'])
        self.u_scale = torch.tensor(example['u_scale'])
        self.mu_loc = torch.tensor(example['mu_loc'])
        self.mu_scale = torch.tensor(example['mu_scale'])
