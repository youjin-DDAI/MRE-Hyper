...
class WaveEquation(object):
    ...
    def __init__(self, rho=1000, omega=None, lambda_=0, detach=True, no_z=True):
    ...
    def body_forces(self, omega, u): 
        ...
        if u.shape!=omega.shape:
            expand_dims = [1] * (u.dim() - 1)
            omega_expanded = omega.view(omega.shape[0], *expand_dims)
            expand_shape = (omega.shape[0], *u.shape[1:])
            omega_expanded = omega_expanded.expand(expand_shape)
            omega = omega_expanded

        return self.rho * omega**2 * u

    def traction_and_body_forces(self, x, u, mu, omega=None):
        '''
        ...
        if self.omega is not None:
            omega=self.omega
        omega = 2 * np.pi * omega #x[:,:1] # radians
        f_trac = self.traction_forces(x, u, mu)
        f_body = self.body_forces(omega, u)
        return f_trac, f_body

    def __call__(self, x, u, mu, omega=None): #x_loc, w, mu_pred, freq
        '''
        ...
        '''
        f_trac, f_body = self.traction_and_body_forces(x, u, mu, omega)
        return f_trac + f_body
...
class HeteroEquation(WaveEquation):
    ...
    def traction_forces(self, x, u, mu):
        ...
        div_grad_u = divergence(grad_u, x, self.no_z)

        if self.detach:
            grad_u = grad_u.detach()
            div_grad_u = div_grad_u.detach()

        grad_mu = jacobian(mu, x, self.no_z)
