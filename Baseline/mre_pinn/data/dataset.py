...
class MREExample(object):
    ...
    def metadata(self):
        ...
        df['limit'] = (df['origin'] + (df['size'] - 1) * df['spacing']).astype(float)
        df['center'] = (df['origin'] + (df['size'] - 1) / 2 * df['spacing']).astype(float)
        df['extent'] = (df['limit'] - df['origin'] + df['spacing']).astype(float)
    ...
    def complex_percentile(self, a, p):
        index = (len(a) - 1) * p/ 100.0
    ...
    def describe(self):
        ...
        for var, array in self.arrays.items():
            if not array.field.has_components:
                ...
                sorted_values = np.sort(values, axis=None, kind='stable', order=None)
                df.loc[(var, comp), '25%'] = self.complex_percentile(sorted_values, 25)  # np.percentile(values, 25)
                df.loc[(var, comp), '50%'] = self.complex_percentile(sorted_values, 50)  # np.percentile(values, 50)
                df.loc[(var, comp), '75%'] = self.complex_percentile(sorted_values, 75)  # np.percentile(values, 75)
